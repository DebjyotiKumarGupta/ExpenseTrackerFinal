...
class ChartController extends GetxController {
  ...
  RxList recentTransactions = [].obs;
  RxList filteredtransactions = [].obs;
  void addtofiltered(data) {
    // filteredtransactions.clear();
    filteredtransactions.value = (data);
    log('recentTransactions: $recentTransactions');
    update();
  }
  ...
  void getTransactionList() async {
    ...
    try {
      ...
      //   filteredtransactions = transactionList.sublist(0, 9);
      // } else {
      recentTransactions.value = transactionList;
      filteredtransactions.value = transactionList;
      // }
      log("recent  ${recentTransactions.toString()}");
      update();
    } catch (e) {
      log("Chandan ${e.toString()}");
    }
  }
}
